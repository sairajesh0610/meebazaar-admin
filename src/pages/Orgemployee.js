...
class Orgemployee extends React.Component{
   ...
   customBody(rowData,column){
    ...
    saveRow = ()=>{
        let isFldMissing = checkReq(this.state.opArr,{forgid:this.props.forgid},this.setData)
        if(isFldMissing){
            ...
            <div style={{position:'relative'}}> 
             {this.state.showSpinner && <AppSpinner spinColor="dark" showText={false} scrHeight={window.screen.height-210}/>}
                <Growl ref={(el) => this.growl = el} sticky={true}/>
                    ...
                    <TableData 
                        screenopt        =  {Object.assign({},this.props.scrOptions,{name:`Employees for ${this.props.parname}`})} 
