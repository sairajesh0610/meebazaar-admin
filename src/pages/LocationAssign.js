...
class LocationAssign extends React.Component{
    ...
    componentDidMount(){
        ...
        getObjData({objtype:this.state.objtype,forgid:this.props.forgid},this.setData,this.growl)
        this.getLocationList();
        //console.log(this.props.forgid)
    }

    componentDidUpdate(prevProps){
        if(this.props.forgid != prevProps.forgid){
            if(this.props.forgid)
                getObjData({objtype:this.state.objtype,forgid:this.props.forgid},this.setData,this.growl)
            else
   ...
   customBody(rowData,column){
    ...
    onChangeDropdown = (val,it) => {
        ...
        if(it.field == 'type'){
            this.editLocationlist(val.listid);
        ...
        console.log(val);
        this.setState({locid:val.locid});
        this.setState({opArr:this.state.opArr})

    }
    getLocationList = () => {
        
        callsvc({objtype:'LOCATION_FLDS'},'getobjdata')
        .then((res)=>{
            if(res.code == '999'){
                let locArr = []

                for(let i=0; i<res.data.length;i++){
                    let a = {locationname:res.data[i].loceng,locid:res.data[i].locid};
                    if(res.data[i].loceng){
                        locArr.push(a);
                    }
            }
            //console.log("locArray",locArr)
                this.setState({locationList:locArr});
    ...
    editLocationlist = (locid) => {
        let locationListArr = [];
        for(let i=0;i<this.state.locationList.length; i++){
            if(this.state.locationList[i]['locid'] == locid){
                locationListArr.push(this.state.locationList[i]);
                console.log("pushed")
            }
        }
        this.setState({selLocationList:locationListArr});
    }
    showEditModal = (op)=>{
        ...
        let opArr = this.state.fldArr.map((it)=>{
                ...
                if(a.field == 'locname'){
                    ...
                    for(let i=0; i<this.state.locationList.length;i++){
                        ...
                        if(a.val == this.state.locationList[i]['locname']){
                            a.val = this.state.locationList[i];
                            this.editLocationlist(this.state.locationList[i].locid)
                            //console.log("a",a);
                            break;
                        }
                    }
    ...
    saveRow = ()=>{
        ...
        a.forgid = this.props.forgid
        a['locid'] = this.state.locid
        //a['appid'] = "MEEBAZAAR";
        a['orgtype'] = this.props.orgtype
        a['typeid'] = this.props.typeid
        a.locname = a.locname.locationname
        console.log('a',a)
            ...
            <div style={{position:'relative'}}> 
             {this.state.showSpinner && <AppSpinner spinColor="dark" showText={false} scrHeight={window.screen.height-210}/>}
                <Growl ref={(el) => this.growl = el} sticky={true}/>
                    ...
                    <TableData 
                        screenopt        =  {Object.assign({},this.props.scrOptions,{name:`Locations for ${this.props.parname}`})} 
                    ...
                    />
                    <Dialog footer={this.renderFooter()} header={'Add/edit Location'} 
                ...
                {this.state.opArr.map((it)=>{
                    ...
                    if(it.formshow){
                        if(it.type == 'text'){
                            ...
                            return (
                                <div className="p-col-12 p-md-6 p-lg-4" key={it.seq}>
                                        <Dropdown id={it.field}  value={it.val} style = {{minWidth:'233px'}} options={it.field == 'locname' ? this.state.locationList:this.state.selLocationList} 
                                        ariaLabel={it.tblheader} onChange={(e) => {this.onChangeDropdown(e.target.value,it)}} 
                                        optionLabel="locationname" tooltip={it.tooltip} tooltipOptions={{position: 'top'}} placeholder={it.tblheader} 
