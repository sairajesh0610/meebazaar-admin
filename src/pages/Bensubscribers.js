...
class Bensubscribers extends React.Component {
    ...
    componentDidMount(){
        ...
        getObjData({objtype:this.state.objtype},this.setData,this.growl,'getsubcustlist');
    ...
    saveRow = ()=>{
        ...
        console.log(a);
        saveDataRow(a,this.state.data,this.setData,this.growl,this.state.idfld,'updcust');
    }
    myUploader = (event) => {
...
{(this.state.selectedRow && this.state.selectedRow.custid) &&
                <TabView style={{marginTop:5}} activeIndex={this.state.activeIndex} onTabChange={(e) => this.setState({activeIndex: e.index})}>
                    ...
                    <CustSubscriptions  parid = {this.state.selectedRow.custid} ismem = {this.state.selectedRow.ismem} 
