...
import AddDialog from './AddDailog';
import {getObjData,getObjFlds,uploadImg,saveDataRow,checkReq,setScrOptions,delDataRow} from '../utils/ServiceCalls';
import Agencyitems from './Agecyitems';
class AgencyTxns extends React.Component {

    constructor(props){
        super(props);
        this.state = {
            objtype:'AGENCY_TNS_FLDS',
            ...
            scrOptions:{name:props.name,icon:'',editopt:false,addopt:false,expopt:false,delopt:false}
        }
    }
    ...
    componentDidMount(){
        ...
        getObjData({objtype:this.state.objtype,cleared:this.props.cleared},this.setData,this.growl);
             ...
             {this.state.showSpinner && <AppSpinner spinColor="dark" showText={false} scrHeight={window.screen.height-210}/>}
                <Growl ref={(el) => this.growl = el} sticky={true}/>
                    ...
                    {(this.state.data.length > 0) &&
                        <Agencyitems 
                        agencyid={this.state.selectedRow.agencyid}
                        transid={this.state.selectedRow.transid}
